...
fn install_gitignore(set: HashSet<String>) {
    ...
    Command::new("curl").args([format!("https://www.toptal.com/developers/gitignore/api/{}", gitignore), ">", format!("./langs/gitignores/{}.txt", filenames)]).spawn().expect("Failed to install gitignore")).expect("failed to reach gitignores");
}
